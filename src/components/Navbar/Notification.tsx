...
import { Notification, NotificationType, useGetImageQuery } from '../../generated/graphql';
...
const NotificationNode:React.FC<Props> = ({notification}) => {
    const [{data: avatar}] = useGetImageQuery({variables: {imageId: notification.triggerUser.avatarId}});
    ...
    return(
        ...
        _hover={{backgroundColor: "hover"}}>
            <Flex>
                <Box mr="10px">
                    <Flex justify="center" align="center" w="60px" h="60px" bg="active" borderRadius="50%">
                    {image}
                    </Flex>
                ...
                <Box>
                    ...
                    <Text color="active">{parseAdvancedDate(notification.createdAt)}</Text>
                </Box>
            </Flex>
        </Box>
    )
